        ...
        DragOn: function (S, opt) { /* Scroll mechanics */
           var isTouchSupported = 'ontouchstart' in window;
            ...
            Sd = (S = $(S)).DragOn = {
                ...
                toggle: function(e) {
                    if (Sd.on) {
                        S.css({ cursor: Sd.opt.cursor }).children('a').on(startEvent,onPrevent).css({ cursor: 'pointer' });
                        S.on({'mousewheel wheel':Sd.onWhell,'mousedown':Sd.onHold});
                        $('body').on({'keydown':Sd.onKeyDown,'keyup':Sd.onKeyDown});
                    } else {
                        S.css({ cursor: '' }).children('a').off(startEvent,onPrevent).css({ cursor: '' });
                ...
                scrollParent : function(Sto) {
                    while ((Sto[0] && Sto[0].nodeType != 1 || Sto.css('overflow')=='visible') || Sto.data('overflow')=='no-dragon' && S[0]!=Sto[0] ) Sto=Sto.parent();
                    return Sto;
                },
                ...
                onHold: function (e) {
                    ...
                    (e.type == startEvent) && (e.preventDefault(), e.stopPropagation());
